...
/*主要流程就是；检查该创建的快照是否已经过时，若过时则直接return。
根据快照范围内log的index，将节点自身的log数组进行修剪。
创建好快照后，节点那些需要持久化的信息也会更新，
像rf.log、rf.lastIncludedIndex、rf.lastIncludedTerm。那么就还需要将这些更新后的信息进行持久化保存，
此时需要持久化保存快照和节点的信息，实验教程是推荐使用persister.SaveStateAndSnapshot函数。
*/
...
func (rf *Raft) InstallSnapshot(serverId int){
	args := RequestInstallSnapShortArgs{}
	reply := RequestInstallSnapShortReply{}
	rf.mu.Lock()
	if rf.state != Leader{
...
func (rf *Raft) sendRequestInstallSnapshot(server int,args *RequestInstallSnapShortArgs,reply* RequestInstallSnapShortReply)bool{
	ok := rf.peers[server].Call("Raft.RequestInstallSnapshot",args,reply)
	return ok
}

func (rf *Raft) RequestInstallSnapshot(args *RequestInstallSnapShortArgs,reply *RequestInstallSnapShortReply) {
	rf.mu.Lock()
	rf.mu.Unlock()
