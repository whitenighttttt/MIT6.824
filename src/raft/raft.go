...
type RequestInstallSnapShortArgs struct{
	Term					int
	LeaderId				int
...
type RequestInstallSnapShortReply struct{
	Term 		int
}
	...
	for !rf.killed(){
		...
		for rf.lastApplied+1 <= rf.commitIndex{
			...
			DPrintf(111, "%s: next apply index=%v lastApplied=%v len entries=%v "+
				"LastLogIndex=%v cmd=%v\n", rf.SayMeL(), i, rf.lastApplied, len(rf.log.Entries),rf.log.LastLogIndex, rf.log.getOneEntry(i).Command)
